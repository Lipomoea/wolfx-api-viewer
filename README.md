...
3. **Linux系统下软件界面、托盘菜单等出现乱码。**  
    ...
    ```
## 数据来源
* 地震预警（CEA/SC/FJ/CWA/JMA）、地震信息（CENC）、地震列表（JMA）、IP定位、NTP时间：[Wolfx Open API](https://wolfx.jp/apidoc)（请注意参考接口文档）
...
* 地震预警（CEA/SC/FJ/CWA/JMA/KMA）、地震信息（CENC/CWA/KMA/USGS/FSSN）、地震列表（CENC/CWA/FSSN）、CENC烈度速报、自然资源部海啸预警、台风信息、IP定位、NTP时间：[FAN Studio API](https://api.fanstudio.tech)
