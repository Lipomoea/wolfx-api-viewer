...
const handleAbout = ()=>{
    ElMessageBox.alert(
        ...
        <div class="about">
            <p>v2.0.0 pre14 变更：WebSocket使用all_eew接口而不是每个数据源单独连接，以减轻前后端数据传输压力。代码结构进行了重大调整，不排除出现重大bug的可能性；优化：代码逻辑优化，地图数据并行加载，GET请求介入逻辑，移除不必要的文件。</p>
            <p>v2.0.0 pre1-pre13 变更：UI重排，使用新的中国和日本地图，暂时移除地震波倒计时功能；新增：同源多个EEW同时展示，中国断层显示，NIED強震モニタ测站显示、震度检出功能，设置默认视野功能，鼠标悬浮提示区域名称，NIED测站回放功能。</p>
        ...
        'wolfx-api-viewer v2.0.0 pre-14',
        {
